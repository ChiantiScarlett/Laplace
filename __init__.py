...
class CashCat:
	def __init__(self, CONNECT_KEY, SECRET_KEY, CURRENCY):
		self.core = XCoinAPI(CONNECT_KEY, SECRET_KEY)
		...
		while True:
			...
			# Buy if current price goes below bid_price
				...
				logging.info("\n[BID CALL COMPLETE]\n"+\
								...
								"\tfee: {:,} KRW)\n".format(float(bid_call[0]['fee'])*float(bid_call[0]['price']))+\
		...
		while True:
			...
			# Buy if current price goes above ask_price
				...
				logging.info("\n[BID CALL COMPLETE]\n"+\
								...
								"\tfee: {:,} KRW)\n".format(float(ask_call[0]['fee'])*float(ask_call[0]['price']))+\
